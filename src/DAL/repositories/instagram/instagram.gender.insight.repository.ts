...
export class InstagramGenderInsightRepository extends BaseRepository<InstagramGenderInsightEntity> {
  constructor(
    @InjectDataSource()
    private readonly _connection: DataSource,
  ) {
    super(_connection.getRepository(InstagramGenderInsightEntity));
  }

  public async getByNameOrGenerateNewEntity(name: string): Promise<InstagramGenderInsightEntity> {
